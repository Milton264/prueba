...
export const settingsSchema = z.object({
  ...
  whatsapp_number: z
    .string()
    .min(8, 'Ingresa el número en formato internacional, solo dígitos')
    .max(20)
    .regex(/^\d+$/, 'Solo dígitos. Ejemplo para Panamá: 50760000000'),
  ...
  request_prefix: z.string().min(1).max(10).regex(/^[A-Z]+$/, 'Solo letras mayúsculas'),
  quotation_prefix: z.string().min(1).max(10).regex(/^[A-Z]+$/, 'Solo letras mayúsculas'),
